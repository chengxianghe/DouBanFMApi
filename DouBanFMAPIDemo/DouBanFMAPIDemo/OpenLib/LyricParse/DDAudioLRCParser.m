...
- (void)setUnits:(NSArray<DDAudioLRCUnit *> *)units
{
    _units = units;
}
...
- (void)setMetadata:(id)value forKey:(NSString *)key
{
    ...
    if (value && key) {
        _medatata[key] = value;
        if ([key isEqualToString:kDDLRCMetadataKeyTI]) {
            _lrc_title = value;
        } else if ([key isEqualToString:kDDLRCMetadataKeyAL]) {
            _lrc_album = value;
        } else if ([key isEqualToString:kDDLRCMetadataKeyAR]) {
            _lrc_autor = value;
        } else if ([key isEqualToString:kDDLRCMetadataKeyBY]) {
            _lrc_create = value;
        } else if ([key isEqualToString:kDDLRCMetadataKeyOFFSET]) {
            _lrc_offset = value;
        } else if ([key isEqualToString:kDDLRCMetadataKeyTIME]) {
            _lrc_time = value;
        } 
    }
}
